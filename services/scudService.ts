...
import { validateScudData } from './scudValidator';
...
/**
 * Generates and downloads the SCUD XLS file.
...
const generateAndDownloadXls = (data: InvoiceData) => {
    const sheetName = "MODELO SCUD";
    
    // Headers
    const headers = [
        "PART_NUMBER", "ESPECIFICAÇÃO_TECNICA", "CODIGO_PRODUTO", "CODIGO_NCM", 
        "DETALHE_PRODUTO", "UNIDADE", "PESO_LIQUIDO", "QUANTIDADE", "VALOR_UNITARIO", 
        "CODIGO_FABRICANTE", "MATERIA_PRIMA", "REFERENCIA_FABRICANTE", "CODIGO_PAIS_FABRICANTE_DESCONHECIDO", 
        "Ato Legal", "Orgão Emissor", "Número", "Ano", "Ex", "Alíquota Ad Valorem (%)", 
        "Ato Legal", "Orgão Emissor", "Número", "Ano", "Ex", "Alíquota Ad Valorem (%)", 
        "Nota Complementar", "NIVEL", "ATRIBUTO", "ESPECIFICAÇÃO", 
        "NIVEL", "ATRIBUTO", "ESPECIFICAÇÃO", 
        "NIVEL", "ATRIBUTO", "ESPECIFICAÇÃO"
    ];
    ...
    items.forEach(item => {
        const rowData = [
            ...
            // Ato Legal 1
            cell(item.legalAct1Type), cell(item.legalAct1Issuer), cell(item.legalAct1Number),
            cell(item.legalAct1Year), cell(item.legalAct1Ex), cell(''), // Alíquota 1 cleared

            // Ato Legal 2
            cell(item.legalAct2Type), cell(item.legalAct2Issuer), cell(item.legalAct2Number),
            cell(item.legalAct2Year), cell(item.legalAct2Ex), cell(''), // Alíquota 2 cleared

            // Attributes
            cell(item.complementaryNote),
            cell(item.attr1Level), cell(item.attr1Name), cell(item.attr1Value),
            cell(item.attr2Level), cell(item.attr2Name), cell(item.attr2Value),
            cell(item.attr3Level), cell(item.attr3Name), cell(item.attr3Value)
        ];
        rows.push(rowData);
    });
    ...
    // Export as XLS (BIFF8)
    XLSX.writeFile(wb, `IMPORTACAO_MODELO_INDUSTRIA_SCUD.xls`, { bookType: 'xls' });
};

/**
 * Main Handler for SCUD Export.
 * Orchestrates Validation -> Error Report OR XLS Generation.
 */
export const handleScudExport = (data: InvoiceData) => {
    // 1. Run Validation
    const errorReport = validateScudData(data);

    if (errorReport) {
        // 2a. If errors, download Text File
        downloadTextReport(errorReport, "relatorio_erros.txt");
    } else {
        // 2b. If valid, download XLS
