...
const cleanData = (obj: any): any => {
  ...
  if (typeof obj === 'string') {
    ...
    if (lower === 'null' || lower === 'n/a' || lower === 'undefined' || lower === 'none' || lower === 'não consta') {
      return null;
    }
    return obj;
  }
...
const safeJsonParse = (text: string): any => {
    ...
    try {
        ...
        logger.warn("JSON Parse failed, attempting aggressive repair...", { error: e });
    }
    ...
    try {
        ...
        logger.warn("Token Balancer failed, attempting Truncation Fallback...", { error: e });
    }
    ...
    for (const candidate of candidates) {
        for (const closer of closers) {
            try {
                ...
                logger.info(`JSON repaired via Truncation at index ${candidate.length} with closer '${closer}'.`);
                return res;
            } catch (ignore) {}
        }
    }
...
const getModelSpecificPrompt = (modelId: string): string => {
  return `
    ...
    CONTEXT: The user is a Customs Broker preparing a Brazilian Import Declaration (DI/DUIMP) using the 'SCUD' industry standard.
    OBJECTIVE: Extract precise data from the Commercial Invoice/Packing List.
    ...
    ### CRITICAL: PART NUMBER & CODE MAPPING RULES (SCUD MODEL)
    ...
    *   *Do not return full names like 'China' or 'United States'. Return 'CHN' or 'USA'.*
    ...
    3. **LINE ITEMS (STRICT LINE-BY-LINE EXTRACTION)**:
       ...
       *   **UNIT NET WEIGHT**: Net weight per unit (if listed).
 ...
 */
export const extractInvoiceData = async (files: FilePart[], modelId: string): Promise<InvoiceData> => {
  const startTime = Date.now();

  // Initialize Gemini API client inside the function to avoid top-level process.env access issues
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

  // Updated Response Schema - Extended for SCUD Model (All Fields)
  const responseSchema = {
    type: Type.OBJECT,
    properties: {
      // --- Identifiers ---
      invoiceNumber: { type: Type.STRING, description: "Commercial Invoice Number (Fatura Comercial)." },
      packingListNumber: { type: Type.STRING, description: "Packing List Number." },
      date: { type: Type.STRING, description: "Issue Date (YYYY-MM-DD)." },
      dueDate: { type: Type.STRING, description: "Payment Due Date (YYYY-MM-DD)." },

      // --- Entities ---
      exporterName: { type: Type.STRING, description: "Exporter/Seller/Issuer Name." },
      exporterAddress: { type: Type.STRING, description: "Exporter Address." },
      importerName: { type: Type.STRING, description: "Importer/Buyer Name. The 'Bill To' entity responsible for the DI." },
      importerAddress: { type: Type.STRING, description: "Importer/Buyer Address." },
      
      // --- Trade & Logistics ---
      incoterm: { type: Type.STRING, description: "3-letter Incoterm Code (e.g., FCA, EXW, FOB). Null if not found." },
      paymentTerms: { type: Type.STRING, description: "Payment Condition (e.g., Net 30, T/T)." },
      countryOfOrigin: { type: Type.STRING, description: "Country of Origin (Made in). MUST BE ISO 3166-1 ALPHA-3 CODE (e.g. CHN, USA, DEU)." },
      countryOfAcquisition: { type: Type.STRING, description: "Country of Acquisition (Sold by). MUST BE ISO 3166-1 ALPHA-3 CODE (e.g. CHN, USA)." },
      countryOfProvenance: { type: Type.STRING, description: "Country of Provenance (Shipped From). MUST BE ISO 3166-1 ALPHA-3 CODE (e.g. HKG, TWN)." },
      totalNetWeight: { type: Type.NUMBER, description: "Total Net Weight (N.W.)." },
      totalGrossWeight: { type: Type.NUMBER, description: "Total Gross Weight (G.W.). Look for 'Total Gross', 'G.W.'." },
      weightUnit: { type: Type.STRING, enum: ["KG", "LB"], description: "Weight Unit. Return 'KG' for Kilograms or 'LB' for Pounds." },
      totalVolumes: { type: Type.NUMBER, description: "Total Quantity of Packages/Volumes." },
      volumeType: { type: Type.STRING, description: "Type of Volume (e.g., Pallets, Cartons)." },

      // --- Financials ---
      currency: { type: Type.STRING, description: "Currency Code (USD, EUR, BRL)." },
      subtotal: { type: Type.NUMBER, description: "Subtotal Value (do not calculate)." },
      freight: { type: Type.NUMBER, description: "International Freight Cost." },
      ...
      grandTotal: { type: Type.NUMBER, description: "Grand Total / Final Amount (do not calculate)." },
      ...
      lineItems: {
        ...
        items: {
          type: Type.OBJECT,
          properties: {
            partNumber: { type: Type.STRING, description: "PART_NUMBER. The Buyer's/Customer's Part Number or SKU. (SCUD Col A)." },
            productCode: { type: Type.STRING, description: "CODIGO_PRODUTO. Secondary internal code. Use only if distinct from Part Number. (SCUD Col C)." },
            manufacturerRef: { type: Type.STRING, description: "REFERENCIA_FABRICANTE. The Seller's/Manufacturer's Part Number/Model. (SCUD Col L)." },
            
            productDetail: { type: Type.STRING, description: "Detail, suffix, or 'Ex' tarifario code." },
            description: { type: Type.STRING, description: "DESCRIPTION. Full commercial description." },
            quantity: { type: Type.NUMBER, description: "QTY. The billed quantity." },
            unitMeasure: { type: Type.STRING, description: "Unit of Measure e.g. PCS, UN, M, KG." },
            unitPrice: { type: Type.NUMBER, description: "UNIT PRICE. Price per single unit." },
            total: { type: Type.NUMBER, description: "Total Line Price." },
            unitNetWeight: { type: Type.NUMBER, description: "Unit Net Weight (if available)." },
            netWeight: { type: Type.NUMBER, description: "ITEM NET WEIGHT. The Total Net Weight for this line item." },
            ncm: { type: Type.STRING, description: "HS Code / NCM (e.g. 8471.30.12). MUST BE NUMERIC (dots allowed). Do NOT put Part Numbers here. If missing, return null." },
            manufacturerCode: { type: Type.STRING, description: "Manufacturer specific code if available." },
            material: { type: Type.STRING, description: "Material composition (e.g. Steel, Plastic)." },
            manufacturerCountry: { type: Type.STRING, description: "Country of Manufacturer. Use ISO 3166-1 ALPHA-3 Code." },
            
            // Regulatory (Ato Legal)
            legalAct1Type: { type: Type.STRING, description: "Regulatory Act Type (Ato Legal)." },
            legalAct1Issuer: { type: Type.STRING, description: "Issuer of Regulatory Act." },
            legalAct1Number: { type: Type.STRING, description: "Act Number." },
            legalAct1Year: { type: Type.STRING, description: "Act Year." },
            legalAct1Ex: { type: Type.STRING, description: "'Ex' code of the Act." },
            legalAct1Rate: { type: Type.NUMBER, description: "Ad Valorem Rate (%)." },

            // Attributes
            complementaryNote: { type: Type.STRING, description: "Complementary Note (Nota Complementar)." },
            
            attr1Level: { type: Type.STRING, description: "Attribute 1 Level." },
            attr1Name: { type: Type.STRING, description: "Attribute 1 Name." },
            attr1Value: { type: Type.STRING, description: "Attribute 1 Value." },
            
            attr2Level: { type: Type.STRING, description: "Attribute 2 Level." },
            attr2Name: { type: Type.STRING, description: "Attribute 2 Name." },
            attr2Value: { type: Type.STRING, description: "Attribute 2 Value." },
          },
    ...
    propertyOrdering: [
        "invoiceNumber", "packingListNumber", "date", "dueDate",
        "exporterName", "exporterAddress", "importerName", "importerAddress",
        "incoterm", "paymentTerms", "countryOfOrigin", "countryOfAcquisition", "countryOfProvenance",
        "totalNetWeight", "totalGrossWeight", "weightUnit", "totalVolumes", "volumeType",
        "currency", "subtotal", "freight", "insurance", "tax", "otherCharges", "grandTotal",
        "lineItems"
    ]
  };
