...
import { INCOTERMS_LIST, COUNTRIES_LIST, VOLUME_TYPES_LIST } from '../../../utils/validationConstants';

export const LogisticsSection: React.FC<SectionProps> = ({ data, handleChange, errors, isReadOnly }) => {
    return (
        <section>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div>
                 <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">Dados Logísticos</h4>
                 
                 <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                     <div className="space-y-1">
                        <label className="text-[10px] uppercase text-slate-400 font-bold ml-1">Peso Líquido Total</label>
                        <div className="flex gap-2">
                             <ValidatedInput 
                                ...
                                onChange={e => handleChange('totalNetWeight', e.target.value)} 
                             ...
                             <select 
                                ...
                                className={`w-20 px-2 rounded-lg bg-slate-50 border-transparent text-sm font-bold text-slate-600 focus:ring-2 focus:ring-brand-500/20 focus:bg-white transition-all outline-none ${isReadOnly ? 'cursor-default' : 'cursor-pointer hover:bg-slate-100'}`}
                             >
                     ...
                     <div className="space-y-1">
                        <label className="text-[10px] uppercase text-slate-400 font-bold ml-1">Peso Bruto Total</label>
                        <div className="flex gap-2">
                            <ValidatedInput 
                                id="totalGrossWeight" 
                                type="number" 
                                ...
                                placeholder="0.00" 
                            />
                             <div className="w-20 flex items-center justify-center bg-slate-50 rounded-lg text-xs font-bold text-slate-400 select-none">
                                {data.weightUnit || 'KG'}
                            </div>
                        </div>
                     </div>
              ...
              <div>
                 <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">Termos de Comércio</h4>
