...
export const EntitiesSection: React.FC<SectionProps> = ({ data, handleChange, errors, isReadOnly }) => {
    return (
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12 relative">
            {/* Visual connector for desktop */}
            <div className="hidden md:block absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 bg-slate-50 p-1.5 rounded-full border border-slate-200 text-slate-300">
                <ArrowRight className="w-4 h-4" />
            </div>

            {/* Exporter */}
            <div className="flex flex-col h-full">
               <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Exportador (Seller)</h4>
            ...
            {/* Importer */}
            <div className="flex flex-col h-full">
               <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Importador (Buyer)</h4>
