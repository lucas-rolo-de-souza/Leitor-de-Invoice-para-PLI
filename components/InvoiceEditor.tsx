      ...
      return (
          <div className="relative">
             <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 mb-6 flex items-center justify-center gap-2 text-purple-700 font-bold text-sm shadow-sm">
                ...
                <span>Visualizando extração original da Inteligência Artificial (Modo Leitura)</span>
             </div>
             {/* Render same content but disabled */}
             <div className="bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden opacity-90 pointer-events-none grayscale-[0.3]">
                <div className="p-8 space-y-10">
                    <HeaderSection data={data} handleChange={handleChange} errors={{}} isReadOnly={true} />
                    <div className="h-px bg-slate-100 w-full" />
                    <EntitiesSection data={data} handleChange={handleChange} errors={{}} isReadOnly={true} />
                    <div className="h-px bg-slate-100 w-full" />
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-3 flex flex-col sm:flex-row items-center justify-between gap-4 sticky top-[72px] z-30 transition-all">
         <div className="flex items-center gap-3 w-full sm:w-auto">
             <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-100 w-full sm:w-auto">
                <div className={`w-2 h-2 rounded-full ${compliancePercentage === 100 ? 'bg-green-500' : 'bg-amber-500'}`}></div>
                <span className="text-xs font-bold text-slate-700">{compliancePercentage}% Conforme</span>
             </div>
         </div>
         ...
         <div className="flex gap-2 overflow-x-auto no-scrollbar w-full sm:w-auto max-w-full pb-1 sm:pb-0">
             {checklist.map(item => (
                 item.status !== 'ok' && (
                     <div key={item.id} className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-50 border border-red-100 text-red-700 text-[10px] font-bold whitespace-nowrap cursor-help" title={item.msg}>
      ...
      <div className="bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden min-h-[800px] relative">
          ...
          <div className="p-6 sm:p-12 space-y-10">
              ...
              <div id="header-section">
                <HeaderSection data={data} handleChange={handleChange} errors={fieldErrors} isReadOnly={isReadOnly} />
              </div>
              ...
              <div id="entities-section">
                <EntitiesSection data={data} handleChange={handleChange} errors={fieldErrors} isReadOnly={isReadOnly} />
              </div>
              ...
              <div id="logistics-section">
                <LogisticsSection data={data} handleChange={handleChange} errors={fieldErrors} isReadOnly={isReadOnly} />
              </div>
              ...
              {/* Footer / Totals */}
              <div className="flex flex-col md:flex-row justify-between items-end gap-8" id="financial-section">
                  <div className="text-[10px] text-slate-400 max-w-xs">
                      <p>Declaramos que esta fatura é verdadeira e correta.</p>
                      <p className="mt-1">Documento processado via AI. Verifique conformidade com Art. 557.</p>
                  </div>
