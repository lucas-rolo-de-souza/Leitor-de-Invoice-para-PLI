...
/**
 * Validates data against strict SCUD Industry Model constraints.
 * Returns a formatted text report if errors are found, or null if valid.
 */
export const validateScudData = (data: InvoiceData): string | null => {
  ...
  items.forEach((item, index) => {
    ...
    if (!item.productCode || item.productCode.trim() === '') {
        lineMessages.push("O campo CODIGO_PRODUTO deve ser formatado com o tipo: Texto");
    }

    if (!item.ncm || item.ncm.trim() === '') {
        lineMessages.push("O campo CODIGO_NCM deve ser formatado com o tipo: Texto");
    }

    // Product Detail: Check existence OR Length
    if (!item.productDetail || item.productDetail.trim() === '') {
        lineMessages.push("O campo DETALHE_PRODUTO deve ser formatado com o tipo: Texto");
    } else if (item.productDetail.length > 4) {
        lineMessages.push(`O campo DETALHE_PRODUTO excedeu o tamnho máximo permitido. Atual:${item.productDetail.length}, Máximo 4.`);
    }
    ...
    if (item.description && item.description.length > 254) {
        lineMessages.push(`O campo DESCRICAO excedeu o tamnho máximo permitido. Atual:${item.description.length}, Máximo 254.`);
    }
    ...
    if (!isNumeric(item.netWeight)) {
        lineMessages.push(`É permitido apenas números para o campo: PESO_LIQUIDO`);
    }

    if (!isNumeric(item.unitPrice)) {
        lineMessages.push(`É permitido apenas números para o campo: VALOR_UNITARIO`);
    }
    ...
    if (!item.manufacturerCode || item.manufacturerCode.trim() === '') {
        ...
        lineMessages.push(`É permitido apenas números para o campo: CODIGO_FABRICANTE`);
    }
