...
import { handleScudExport } from './services/scudService';
...
import { FileText, Download, RotateCcw, AlertTriangle, Cpu, Sparkles, Scale, FileSpreadsheet, GitCommit, FileJson, X, History } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [originalData, setOriginalData] = useState<InvoiceData>(initialInvoiceData); // AI Result (Immutable)
  ...
  const handleFilesSelect = async (selectedFiles: File[]) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Erro desconhecido ao processar arquivos.";
      setError(msg);
  ...
  const handleExportExcel = () => { learn(); exportToExcel(data); };
  // Using the new service for SCUD
  const handleExportScudButton = () => { learn(); handleScudExport(data); };
  const handleExportPDF = () => { learn(); exportToPDF(data); };

  const handleReset = () => {
    if (window.confirm('Voltar para o início? Dados não salvos serão perdidos.')) {
  ...
  const validationErrors = useMemo(() => 
    generateValidationErrors(activeData, { incoterms: INCOTERMS_LIST, currencies: CURRENCIES_LIST, countries: COUNTRIES_LIST }), 
  [activeData]);
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900">
        <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
           <div className="flex items-center gap-2">
             ...
             <div>
               <h1 className="text-lg font-bold tracking-tight text-slate-900">Leitor de Faturas AI</h1>
    ...
    <div className="min-h-screen bg-slate-100/50 flex flex-col font-sans text-slate-900">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-3 flex justify-between items-center sticky top-0 z-[40]">
         ...
         <div className="flex items-center gap-4 relative z-10">
             <button onClick={handleReset} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600 transition-colors" title="Voltar para Upload">
             ...
             <div className="flex flex-col">
                <h1 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                    ...
                    {!isValid && <AlertTriangle className="w-3.5 h-3.5 text-amber-500" />}
                </h1>
                <span className="text-[10px] text-slate-500 flex items-center gap-1">
                   <FileText className="w-3 h-3" /> {files.length} arquivo(s) processado(s)
                </span>
             </div>
         </div>
         ...
         <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
            <div className="pointer-events-auto"> {/* Re-enable clicks specifically for the buttons */}
                <VersionBar showOriginal={showOriginal} onToggle={setShowOriginal} />
            </div>
         </div>
         ...
         <div className="flex items-center gap-2 relative z-10">
             <div className="hidden md:flex items-center gap-2 mr-2">
                <button onClick={handleExportPDF} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 transition-colors" title="PDF">
                    <FileText className="w-4 h-4" />
                </button>
                <button onClick={handleExportScudButton} className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 transition-colors" title="SCUD (XLS + Validação)">
             ...
             >
                <Download className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Exportar Excel</span>
             </button>
         </div>
      </header>
      ...
      <footer className="fixed bottom-0 w-full bg-white border-t border-slate-200 px-6 py-2 z-[50] flex justify-between items-center text-[10px] text-slate-400">
         <div className="flex items-center gap-3">
             <button onClick={() => setShowChangelog(true)} className="hover:text-slate-600 flex items-center gap-1">
                 <GitCommit className="w-3 h-3" /> v{APP_VERSION}
             </button>
             <button onClick={() => logger.downloadLogs()} className="hover:text-slate-600 flex items-center gap-1">
                 <FileJson className="w-3 h-3" /> Logs
             </button>
             <button onClick={() => setShowLegal(true)} className="hover:text-slate-600 flex items-center gap-1">
                 <Scale className="w-3 h-3" /> Legal
             </button>
         </div>
         <div className="flex items-center gap-3">
             <div className="flex items-center gap-1.5">
                 <div className={`w-1.5 h-1.5 rounded-full ${ncmService.getStatus().isReady ? 'bg-green-500' : 'bg-amber-400'}`}></div>
                 <span>NCM DB</span>
             </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col">
                  <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                      <h3 className="font-bold text-slate-800 flex items-center gap-2"><History className="w-4 h-4" /> Changelog</h3>
                      <button onClick={() => setShowChangelog(false)}><X className="w-4 h-4" /></button>
                  </div>
                  <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
                      {CHANGE_LOG.map((log, i) => (
                          <div key={i} className="pl-4 border-l-2 border-slate-100 relative">
                              <div className={`absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full ${i === 0 ? 'bg-slate-900' : 'bg-slate-300'}`}></div>
                              <h4 className="font-bold text-slate-800 text-sm mb-1">v{log.version} <span className="text-slate-400 font-normal text-xs ml-2">{log.date}</span></h4>
                              <ul className="space-y-1">{log.changes.map((c, idx) => <li key={idx} className="text-xs text-slate-600">• {c}</li>)}</ul>
                          </div>
                      ))}
                  </div>
              </div>
